...
// 1. Remove ONLY the Cloudflare script at the very end
let html = origHtml.replace(/<script>\(function\(\)\{function c\(\).*?<\/script><\/body>/s, '</body>');

// 2. Update visible title in <head> without breaking Flight JSON
html = html.replace(/<title>.*?<\/title>/, '<title>UnionFam Life Lab | 3D Studio &amp; Life Design</title>');

// 3. Inject our UnionFam Life Lab HUD and Modal
...
console.log('Restored pristine Next.js Flight stream in index.html with clean UnionFam overlay');
